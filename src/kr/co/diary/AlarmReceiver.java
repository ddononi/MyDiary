...
public class AlarmReceiver extends BroadcastReceiver {
	...
	public void onReceive(final Context context, final Intent intent) {
		...
		int idx = intent.getIntExtra("alarmIdx", -1);
		updateAlarmed(context, idx);
	}
	...
	private void showNotification(final Context context,
		...
		CharSequence message = "일정 알림" + sp.getString("beforeAlarm", "10")
				+ "분전 입니다."; // 알림 내용
	 ...
	 * 발생된 알람은 제거
	 */
	private void updateAlarmed(Context context, final int idx) {
		// TODO Auto-generated method stub
		DBHelper mydb = new DBHelper(context);
		SQLiteDatabase db = mydb.getWritableDatabase();
		ContentValues cv = new ContentValues();
		cv.put("alarm", 0); // 오늘 날짜로 update
		db.update(DBHelper.SCHEDULE_TABLE, cv, "no=?",
				new String[] { idx + "", });
		db.close();
	}
